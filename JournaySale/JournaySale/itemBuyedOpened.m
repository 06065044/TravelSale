...
//@property(strong,nonatomic)UIButton *lookHistory;
@end
@implementation itemBuyedOpened
...
//        [self.contentView addSubview:_itemDistance];
        ...
        _bounsButton.frame = CGRectMake(CGRectGetMinX(_itemTitle.frame),CGRectGetMaxY(_itemDesc.frame)+10,CGRectGetWidth(_itemTitle.frame)/2,25);
        [_bounsButton setTitle:@"开奖明细" forState:UIControlStateNormal];
         [_bounsButton setBackgroundColor:[UIColor orangeColor]];
        [_bounsButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_bounsButton addTarget:self action:@selector(goToHistory) forControlEvents:UIControlEventTouchUpInside];
